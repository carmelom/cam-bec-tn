...
class TreeModel(object):
    ...
    #imagefilematch = re.compile(r'\d{8}-(?P<name>.*)-(?P<nr>\d{4}).sis') up to 2017-12-04 when sis fimename changed date format
    
    imagefilematch = re.compile(r'([0-9T\-]*)-(?P<name>.*)-(?P<nr>\d{4}).sis')
    ...
    def createdaysubtree(self, daydir):
        ...
        for filename in names:
            ...
            imagelist.append(DirListEntry(name = m.group('nr'),
